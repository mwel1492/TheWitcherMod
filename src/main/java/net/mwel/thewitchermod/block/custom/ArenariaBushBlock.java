...
public class ArenariaBushBlock extends PlantBlock {
    public ArenariaBushBlock(Settings settings) {
        super(settings);
    ...
    public ItemStack getPickStack(BlockView world, BlockPos pos, BlockState state) {
        return new ItemStack(ModBlocks.arenaria_bush);
    }
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        int i = state.get(AGE);
        if (i < 6 && random.nextInt(6) == 0 && world.getBaseLightLevel(pos.up(), 0) >= 9) {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (i > 5) {
            ...
            ArenariaBushBlock.dropStack(world, pos, new ItemStack(ModItems.arenaria, j + (bl ? 1 : 0)));
